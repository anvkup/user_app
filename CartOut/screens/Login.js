...
export default function Login (props){
    ...
    async function verifyOtp() {
        ...
        const response = await fetch(`http://20.193.147.19:80/api/users/verifyOTP?phone=${phone}&otp=${otp2}`)
        const data = await response.json()
    ...
    return (
        <SafeAreaView style={{display: 'flex', position: 'relative', justifyContent: 'center', width: '100%', height: '100%', paddingHorizontal: 30}}>
            ...
            <Image source={require('../assets/cartoutLogo.png')} style={{width: 200, height: 200, top: 0, alignSelf: 'center', marginBottom: 75, marginTop: -75}} />
            {
                otpSent ? 
                <View>
                    <Input placeholder='OTP' label='Enter OTP sent to your Phone' maxLength={4} keyboardType='numeric' value={otp2} onChangeText={(i)=>{setotp(i)}} style={{marginBottom: 30}} />
